...
try:
    if sys.argv[1].strip() != password[2].rstrip():
        print('Password is not correct')
        exit()
    elif sys.argv[1].strip() == password[2].rstrip():
...
while True:
    ...
    if '52f3149496de55aded000410' in ids:
        with open('.passwords', 'r') as passwords:
            passwords = passwords.readlines()
            print('You have succesfully completed the challenge\n\
            Here is the password for the next level: {}'.format(ps.passwords[3]))
            break
